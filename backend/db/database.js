...
//test user data
let users = [
  {email: 'johndoe@test.com', password: 'johndoe', hostid: 47},
  {email: 'janedoe@test.com', password: 'janedoe', hostid: 74},
  {email: 'jamessmith@test.com', password: 'jamessmith', hostid: 77},
]

//loop through test van data and add each van to db
// function addVanData() {
//   testVanData.forEach(vanData => {
//     db.run(
...
//loop through users test data and add each user to db
// function addUsers() {
//   users.forEach(user => {
//     db.run(
//       `INSERT INTO users (email, password, hostId) VALUES (?, ?, ?)`,
//       [user.email, user.password, user.hostid],
//       function(err) {
//         if (err) {
//           return console.error(err.message);
//         }
...
// db.run(`ALTER TABLE users ADD COLUMN hostId TEXT`);
